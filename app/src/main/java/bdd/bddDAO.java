...
//--------------------------------------------------------------------------------------------------
    ...
    public Entreprise cursorToEntreprise(Cursor c){

        if (c.getCount() == 0)
            return null;
        c.moveToFirst(); //on se place sur le premier élément
        Entreprise uneEntreprise = new Entreprise(null,null,null); //On créé un étudiant
        //on lui affecte toutes les infos grâce aux infos contenues dans le Cursor
        uneEntreprise.setId(c.getInt(NUM_COL_ID_ENTREPRISE));
    ...
    public static String[] cursorToEntreprise2(Cursor c){
        ...
        if (c.getCount() != 0) {
            ...
            while (!c.isAfterLast()) {
                ...
                uneEntreprise = c.getString(NUM_COL_NOM_ENTREPRISE);
...
//--------------------------------------------------------------------------------------------------
    ...
    public Tuteur cursorToTuteur(Cursor c){
